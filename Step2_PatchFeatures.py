...
PostivePatch_Path = config.PostivePath
NegativePatch_Path = config.NegativePath
Featurecsv_Path = config.Featurecsv_Path
from PatchClassify_config import secfea_list_3 as secfea_list# 更改特征组合
# keyword definition. 定义关键字
...
def main():
	Patch_fea_Flag = sys.argv[1]  # 0, 1-30 # 0 is negative patch, 1-30 is positive patch
	if 1 == int(float(Patch_fea_Flag)): # 1 is positive patch
		print(os.listdir(PostivePatch_Path))
		cnt = 1
		for root, ds, fs in os.walk(PostivePatch_Path):
			for file in fs:
				filename = os.path.join(root, file)
				print(cnt, filename)
				if Judge_patchlines(file_path= filename)==0:
					continue
				Process(filename, '1') #打标签为1
				cnt += 1
	else:						# 0 is negative patch
		cnt = 1
		# for root, ds, fs in os.walk(negativePatch_Path + 'commit' + folder.zfill(2)):
		print(os.listdir(NegativePatch_Path))
		for root, ds, fs in os.walk(NegativePatch_Path):
			print(NegativePatch_Path)
			for file in fs:
				filename = os.path.join(root, file)
				print(cnt, filename)
				# 判断patch行数是否大于2000 大于2000的直接去除
				if Judge_patchlines(file_path= filename)==0:
					continue
				Process(filename, '0') #打标签为0
				cnt += 1
	if os.path.exists(Featurecsv_Path) != True:
		os.makedirs(Featurecsv_Path)
	if 1 == int(float(Patch_fea_Flag)): # postive /security
		Write2File(Featurecsv_Path + 'feature' +"_positive_" +Patch_fea_Flag.zfill(2) + '.csv')
	else:			# negative
		Write2File(Featurecsv_Path + 'feature' + "_negative_" + Patch_fea_Flag.zfill(2) + '.csv')

	return

def Judge_patchlines(file_path=None):
	'''
	set a value ,it is used to exclude big patch.
	'''
...
def Process(filename, goldtruth):
	...
	if "line count" in secfea_list:
		GetLineInfo(deletion, addition)
	if "char count" in secfea_list:
		GetCharInfo(deletion, addition)
	if "loop keywords" in secfea_list:
		GetLoopInfo(deletion, addition)
	if "memory keywords" in secfea_list:
		GetMemInfo(deletion, addition)
	if "if keywords" in secfea_list:
		GetIfInfo(deletion, addition)
	if "jump keywords" in secfea_list:
		GetJumpInfo(deletion, addition)
	
	if "arithmetic operations" and "relational operations" and "logical operations" and "bit operations" in secfea_list:
		GetAriRelLogBit(deletion, addition)

	GetGlobalSim(deletion, addition) # 补丁全局的相似性
	if "dictionary keywords" in secfea_list:
		GetDirInfo(deletion, addition)
	if "C keywords" and "C++ keywords" in secfea_list:
		del_var_list, add_var_list = GetCallVar_count(deletion, addition) # 与C和c++关键字有关的变量和函数
		GetCapInfo(del_var_list, add_var_list) # 这个暂时不清楚含义，该不该统计
	if "race keywords" in secfea_list:
		GetRaceInfo(deletion, addition)
	
	if "not keywords" in secfea_list:
		GetNotZero(deletion, addition)
	
	if "preprocess count" in secfea_list:
		GetPreprocess(deletion, addition)# - 和 + 行的数量差值 统计 
	return
...
def GetCallVar_count(deletion, addition):
	# call and var
	del_func_list = []
	del_var_list = []
	add_func_list = []
...
def GetPreprocess(deletion, addition): # 计算+ 和 -之间的数量差值
...
def Write2File(filename):
	...
	if "diff count" in secfea_list:
		dset['diff_num'] = diff_num
	if "hunk count" in secfea_list:
		dset['hunk_num'] = hunk_num
	if "func count" in secfea_list:
	...
	if "arithmetic operations" and "relational operations" and \
			"logical operations" and "bit operations" in secfea_list:
