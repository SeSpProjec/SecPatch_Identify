...
_DTYP_  = 0 #  0 : maintain both diff code and context code.
...
_TWIN_  = 1 #  0 : only twin neural network. #fixme 可以测试出是否是因为commit message的信息不足，导致效果不好
...
def Read_Label_Data():
    ...
    def ReadDiffLines(filename):
        ...
        for i in range(numLines):
            if ".java" in lines[i] or ".py" in lines[i] or ".jsp" in lines[i] or ".cs" in lines[i] or ".php" in lines[i]:
                return []
...
def GetDiffProps(data):
    ...
    def GetClangTokens(line):
        ...
        # if ".c" in line:
        #     tu = idx.parse('tmp.c', unsaved_files=[('tmp.c', RemoveSign(line))], options=0)
        # else:
        try:
            tu = idx.parse('tmp.cpp', args=['-std=c++11'], unsaved_files=[('tmp.cpp', RemoveSign(line))], options=0) # fixme    raise TranslationUnitLoadError("Error parsing translation unit.") clang.cindex.TranslationUnitLoadError: Error parsing translation unit.
        except Exception as e:
            print("这里是出错的地方：",line)
            logger.info(
                '[INFO] <error> error in' + line )
            tokens.append("error")
            tokenTypes.append(0)
            diffTypes.append(0)
...
######################################### process diff file
    ...
    # 2.get the diff file properties.   # | (not _DEBUG_)
    if (not os.path.exists(tempPath + _FILEFLAG_+ 'props.npy')):
        diffProps = GetDiffProps(dataLoaded)
    else:
